...
import com.walgreens.rxi.rpl.config.KafkaProperties;
...
class ReplenishmentKafkaResourceIT {
    ...
    void setup() {
        ...
        ReplenishmentKafkaResource kafkaResource = new ReplenishmentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/replenishment-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/replenishment-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
